...
async function main() {
  ...
  // Fetch recent transmits for this aggregator
  const txSql = `select ts from ${schema}.oracle_transmission where chain_id=$1 and lower(oracle_addr)=$2 order by block_number desc limit $3`;
  const txRes = await pool.query(txSql, [chainId, aggregator, Math.max(1, Math.min(1000, limit))]);
  // Fetch recent liquidations for this market
  const liqSql = `select ts from ${schema}.liquidation where chain_id=$1 and lower(market_id)=$2 order by block_number desc limit $3`;
  const liqRes = await pool.query(liqSql, [chainId, marketId, Math.max(1, Math.min(1000, limit))]);
  await pool.end();
  ...
  let txCovered = 0;
  for (const t of transmits) if (covered(t, sessions)) txCovered++;
  let liqCovered = 0;
  for (const t of liqs) if (covered(t, sessions)) liqCovered++;
